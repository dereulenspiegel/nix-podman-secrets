    ...
    in
    {
      ...
      nixosModules.default = (self: { lib, config, pkgs, ... }: {
        ...
        system.activationScripts.syncNixPodmanSecrets = (lib.stringAfter ([ "specialfs" "users" "groups" "setupSecrets" ])) ''
          ...
          PATH=$PATH:${lib.makeBinPath [ pkgs.podman self.packages.${pkgs.system}.nix-podman-secrets ]} ${self.packages.${pkgs.system}.nix-podman-secrets.outPath}/bin/nix-podman-secret-populate
        '';
